...
def start_game():
    ...
    def start_local_game(starrail):
        # switch_to_game() 尝试切换到游戏窗口并激活；返回 False 说明游戏窗口不存在（游戏未运行），需要冷启动
        if not starrail.switch_to_game():
            # ========== 分支一：游戏未运行 → 完整冷启动流程 ==========

            # 若开启了"自动设置分辨率"：先把游戏分辨率改为 1920x1080 并关闭 HDR，
            if cfg.auto_set_resolution_enable:
                starrail.change_resolution(1920, 1080)
                starrail.change_auto_hdr("disable")

            # 若开启了"自动战斗检测"：修改游戏设置，进入战斗后自动启用"自动战斗"
            if cfg.auto_battle_detect_enable:
                starrail.change_auto_battle(True)

            # 真正拉起游戏进程（通过配置/注册表找游戏路径启动）；失败直接抛错，交给外层 stop+重试
            if not starrail.start_game_process():
                raise RuntimeError("启动游戏失败")
            time.sleep(10)  # 等待游戏进程启动完成

            # 轮询等待游戏窗口出现并成功切换：每 1 秒尝试一次，最长等 360 秒（6 分钟）
            if not wait_until(lambda: starrail.switch_to_game(), 360):
                # 超时说明游戏没能正常打开窗口：先恢复用户原本的分辨率/HDR 设置（不留垃圾配置），再抛超时异常
                starrail.restore_resolution()
                starrail.restore_auto_hdr()
                raise TimeoutError("切换到游戏超时")

            time.sleep(10)  # 窗口出现后再等 10 秒，让游戏加载完启动画面附近的界面
            # 此时游戏内分辨率设置已生效，把启动前临时改的分辨率和 HDR 恢复为用户原本的配置
            starrail.restore_resolution()
            starrail.restore_auto_hdr()
            # starrail.check_resolution_ratio(1920, 1080)
            # 校验当前分辨率确实是 1920x1080（修改分辨率偶尔会失败，这里做兜底确认）
            starrail.check_resolution(1920, 1080)

            # 关键一步：轮询"点击进入"按钮——每 1 秒截屏识别一次，期间若出现热更新确认、网络异常重启、
            # 登录过期、B 服登录等弹窗会被 check_and_click_enter 内部自动处理，
            # 直到成功点进游戏主界面为止；最长等待 cfg.start_game_timeout 分钟
            if not wait_until(lambda: check_and_click_enter(), cfg.start_game_timeout * 60):
                raise TimeoutError("查找并点击进入按钮超时")

            # 偶现加载慢时分辨率修改不成功，进入游戏前增加一次分辨率检查，确保后续识别坐标准确
            starrail.check_resolution(1920, 1080)

            time.sleep(10)  # 等待游戏加载动画/过场结束，避免截图识别到加载画面
            # 点一下鼠标（无操作输入）：修复 B 服客户端偶发卡界面的问题
            # https://github.com/moesnow/March7thAssistant/discussions/321#discussioncomment-10565807
            auto.press_mouse()
        else:
            # ========== 分支二：游戏已在运行 → 直接复用现有进程 ==========

            # starrail.check_resolution_ratio(1920, 1080)
            # 校验当前分辨率是否为 1920x1080，不符合脚本基准时会提示/尝试处理
            starrail.check_resolution(1920, 1080)
            # 若开启了"自动设置游戏路径"：从系统进程列表反查游戏 exe 的真实运行路径，
            # 若与配置中记录的路径不一致（游戏更新/迁移后路径可能变化）就自动回写配置
            ...
            time.sleep(1)  # 短暂等待，让窗口切换和分辨率检查结果稳定
